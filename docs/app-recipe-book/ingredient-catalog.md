...
- **Setup notes:** The brick ships Firebase Crashlytics and a `wireCrashHandlers()` call in `bootstrap.dart` that routes both Flutter framework errors and Dart zone errors to Crashlytics. Run `bash tool/setup_firebase.sh` from the **generated app's** project root (the script ships in `__brick__/tool/`, so it lands in your new project after `mason make` — not in the brick repo) to configure FlutterFire CLI, add `google-services.json` / `GoogleService-Info.plist`, and apply the Gradle plugin patcher. No additional wiring required after that script.
